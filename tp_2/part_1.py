...
def tp2():
    ...
    for mode_idx in range(2):
        for n in N:
            for b in B:
                ...
                print(f"Entreno PT2 = N:{n} B:{b} Error:{error} {modo}")
    ...
    mejor_resultado_pt3 = {"error_min": float("inf"), "model": {}, "result": []}
    for r in modelos_entrenados_pt3:
        if r["error_min"] <= mejor_resultado_pt3["error_min"]:
            mejor_resultado_pt3["error_min"] = r["error_min"]
            mejor_resultado_pt3["model"] = r

    hiperparams = mejor_resultado_pt3["model"]
    w = hiperparams["w"]
    b = hiperparams["b"]
    tanh_mode = hiperparams["tanh_mode"]
    test_data = hiperparams["test_data"]
